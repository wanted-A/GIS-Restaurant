...
def get_row(params, business_type):
    ...
    try:
        response = json.loads(requests.get(url, params=params).text)[business_type][1].get("row")
        return response
    except Exception as e:
...
def preprocess_data(raw_data):
    ...
    for field in FIELDS:
        try:
            if field in ["LOCPLC_AR", "TOT_FACLT_SCALE", "MALE_ENFLPSN_CNT", "FEMALE_ENFLPSN_CNT", "TOT_EMPLY_CNT"]:
                ...
                raw_data[field] = raw_data[field] if bool(re.match(r"\d{4}-\d{2}-\d{2}", raw_data[field])) else f"{raw_data[field][:4]}-{raw_data[field][4:6]}-{raw_data[field][6:8]}"
...
def save_raw_data(total_list, page):
    ...
    for raw_data in total_list:
        try:
            ...
            if Restaurant.objects.filter(restaurant_code=restaurant_code).exists(): continue
